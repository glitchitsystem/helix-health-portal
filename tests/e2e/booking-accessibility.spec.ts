...
test.describe('Booking wizard — accessibility', () => {
  ...
  test('step 1 (Choose Type) has no critical or serious violations', async ({ page }) => {
  ...
  test('step 2 (Pick Time) has no critical or serious violations', async ({ page }) => {
    const bookingPage = new AppointmentBookingPage(page);
    await bookingPage.goto();
  ...
  test('step 3 (Confirm) has no critical or serious violations', async ({ page }) => {
    const bookingPage = new AppointmentBookingPage(page);
    await bookingPage.goto();
    await bookingPage.selectAppointmentType('Follow-up');
    ...
    await page.getByRole('button', { name: /\d{1,2}:\d{2}|\bam\b|\bpm\b/i }).first().click();
